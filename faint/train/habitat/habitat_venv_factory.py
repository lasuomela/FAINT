...
"""
Included here because the version in the official Habitat repo had a
bug that causes the number of scenes not to be handled correctly
when the number of scenes is less than the number of environments.
This is a copy of the file from the master repo with the bug fixed.
"""
    ...
    def construct_envs(
        ...
        if len(scenes) < num_environments:
            ...
            if enforce_scenes_greater_eq_environments:
                ...
                # Bugfix here: the following lines had a wrong level of indentation
                for scene in scenes:
