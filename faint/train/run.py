...
# This sets the omegaconf
from faint.train.habitat import dagger_trainer
...
def main(cfg: "DictConfig"):
    ...
    # Resolve variable interpolations here
    with read_write(cfg):
        OmegaConf.resolve(cfg)

    execute_exp(cfg, f"eval_{cfg.habitat_baselines.eval.eval_type}" 
                if cfg.habitat_baselines.evaluate else "train")
...
def execute_exp(config: "DictConfig", run_type: str) -> None:
    ...
    print('Setting NCCL_P2P_LEVEL to PXB')
    ...
    if run_type == "train":
        trainer.train()
    elif run_type == "eval_online":
        trainer.eval_online()
    elif run_type == "eval_offline":
        trainer.eval_offline()
